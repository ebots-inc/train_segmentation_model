...
class SegmentationDataset:
    ...
    def __init__(self, images_dir: str = None, class_key: str = 'object',
        ...
        if 'rpn_anchor_scales' in self.kwargs:
            self.anchors = utils.generate_pyramid_anchors(self.kwargs['rpn_anchor_scales'],
                                                        self.kwargs['rpn_anchor_ratios'], self.backbone_shapes,
                                                        self.kwargs['backbone_strides'],
                                                        self.kwargs['rpn_anchor_stride'])
        else:
            self.anchors = utils.generate_pyramid_anchors_from_xy_scales(self.kwargs['rpn_anchor_scales_x'],
                                        self.kwargs['rpn_anchor_scales_y'], self.backbone_shapes,
                                        self.kwargs['backbone_strides'],
                                        self.kwargs['rpn_anchor_stride'])
        if self.verbose:
    ...
    def get_points_from_annotation(self, annotation_key: str) -> Tuple[list, list]:
        ...
        for region in _region_list:
            ...
            if len(region['region_attributes'].keys()) == 0:
                ...
                print("Class names in dataset: ",class_name, self.classes_dict[class_name])
                if len(class_name) == 0:
    ...
    def __getitem__(self, idx: int):
        ...
        # Use processed masks to create smaller bboxes each of length 32x32.
        # For each cable instance: Use left most pixel and right most pixel mask location.
        # left_x, left_y; right_x, right_y. See which direction is the cable going
        # if the cable is changing along x direction. Calculate left_x+32/2 and corresponding left_y position from the cross-section.
        # Place box at this center and move 8 pixels along x direction do the same, till left_x+32>= right_x
        # At each pixel location measure the cross-section of the cable.
        
        # def break_down_cable_bbox_mask():

        # Break down cable bboxes and masks.
        # This algorithm doesnt work for heavily curved cables since it doesnt follow curve of cable to fit bboxes.
        # Aim: 
        # Equal sampling: For each instance of cable pick some samples. Is it necessary thou
        ...
        smaller_masks = []
        smaller_classes = []
        cable_instances_start_end_indices = []
        for instance_id, cls_id in enumerate(proc_class_ids):
            if cls_id==3:
                ...
                if along_y_axis:
                    start_y = min(bbox[0], bbox[2])
                    end_y = max(bbox[0], bbox[2])
                    start_x = -1
                    end_x = -1
                    if start_y==bbox[0]:
                        start_x = bbox[1]
                    ...
                    start_idx = len(smaller_bboxes)
                    for cur_y in range(start_y, end_y, 4):
                        mask = np.zeros((512,512), dtype=bool)
                        mid_y = min(511, cur_y+16)
                        # measure crossection mid
                        mask_indices = np.where(proc_masks[mid_y,:,instance_id])[0]
                        if mask_indices.size==0:
                            # print("XXXXXXXXXX Zero mask indices seen at mid_y: ", mid_y)
                            continue
                        ...
                        if cur_y+32>end_y:
                            break
                    end_idx = len(smaller_bboxes)
                    cable_instances_start_end_indices.append([start_idx, end_idx])
                else:
                    start_x = min(bbox[1], bbox[3])
                    end_x = max(bbox[1], bbox[3])
                    start_y = -1
                    end_y = -1
                    if start_x==bbox[1]:
                        start_y = bbox[0]
                    ...
                    start_idx = len(smaller_bboxes)
                    for cur_x in range(start_x, end_x, 4):
                        mask = np.zeros((512,512), dtype=bool)
                        mid_x = min(511, cur_x+16)
                        # measure crossection mid
                        mask_indices = np.where(proc_masks[:, mid_x, instance_id])[0]
                        if mask_indices.size==0:
                            # print("XXXXXXXXXX Zero mask indices seen at mid_x: ", mid_x)
                            continue
        ...
        if len(smaller_classes)>0:
            non_cable_instances = np.logical_and(proc_class_ids>0, proc_class_ids!=3)
            cable_instances_start_end_indices = np.array(cable_instances_start_end_indices)+non_cable_instances.shape[0]
            proc_class_ids = np.concatenate((proc_class_ids[non_cable_instances], smaller_classes), axis=0)
            smaller_masks = np.array(smaller_masks)
            # print("shape of broken down masks... ",smaller_masks.shape)
            smaller_masks = np.swapaxes(smaller_masks, 0,1)
            smaller_masks = np.swapaxes(smaller_masks, 1,2)
            proc_masks = np.concatenate((proc_masks[:,:,non_cable_instances], smaller_masks), axis=-1)
            bboxes =  np.concatenate((bboxes[non_cable_instances], smaller_bboxes), axis=0)
        ...
        return proc_image, proc_masks, proc_class_ids, bboxes, cable_instances_start_end_indices, image_meta, \
...
class DataLoader(Sequence):
    ...
    def __init__(self, dataset, detection_targets: bool = False, shuffle: bool = True, seed: int = 42,
                 name: str = 'dataloader', cast_output: bool = True, return_original: bool = False, return_overlaps: bool = False, **kwargs):
        ...
        self.return_overlaps = return_overlaps
    ...
    def generate_batch(self, index: int):
        ...
        while gen_batch < self.batch_size:
            ...
            """
            # print("idx", gen_batch, index, len(self.indexes))
            # img_dir = "/home/wjohn/work/git/segmentation/networks/mrtf2/gt_pics_smaller_cable_boxes/"
            # with open(img_dir+"img_"+str(index)+".npy", 'wb') as f:
            #     np.save(f, original_image)
            # with open(img_dir+"img_"+str(index)+"_bboxes"+".npy", 'wb') as f:
            #     np.save(f, original_bboxes)
            # with open(img_dir+"img_"+str(index)+"_masks_ids"+".npy", 'wb') as f:
            #     np.save(f, original_masks_array)
            # with open(img_dir+"img_"+str(index)+"_class_ids"+".npy", 'wb') as f:
            #     np.save(f, original_class_ids)

            # with open(img_dir+"img_"+str(index)+"_resized.npy", 'wb') as f:
            #     np.save(f, image)
            # with open(img_dir+"img_"+str(index)+"_bboxes"+"_resized.npy", 'wb') as f:
            #     np.save(f, gt_boxes)
            # with open(img_dir+"img_"+str(index)+"_masks_ids"+"_resized.npy", 'wb') as f:
            #     np.save(f, gt_masks)
            # with open(img_dir+"img_"+str(index)+"_class_ids"+"_resized.npy", 'wb') as f:
            #     np.save(f, gt_class_ids)
            ...
            # RPN Targets
            rpn_match, rpn_bbox, overlaps = utils.build_rpn_targets(
                anchors=self.anchors,
                gt_class_ids=gt_class_ids,
                gt_boxes=gt_boxes,
                rpn_train_anchors_per_image=self.kwargs['rpn_train_anchors_per_image'],
                rpn_bbox_std=self.kwargs['rpn_bbox_std_dev'],
                cable_instances_start_end_indices=cable_instances_start_end_indices
            )

            # Mask R-CNN Targets
            if self.random_rois:
                rpn_rois = utils.generate_random_rois(image.shape, self.random_rois, gt_boxes, gt_class_ids)
            ...
            # Add to a batch
            if self.return_overlaps:
                batch_overlaps.append(overlaps)
