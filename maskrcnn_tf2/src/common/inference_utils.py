...
def process_input(input_image, config, idx, preprocess_transform=lambda x: x / 255):
    """
    ...
    # Image transformation, change this.
    if preprocess_transform:
        input_image = preprocess_transform(image=input_image)["image"]
    # input_image.tofile("/home/wjohn/work/git/segmentation/networks/mrtf2/norm_image_"+idx+".dat")
    # Resize image
    ...
    # resized_image.tofile("/home/wjohn/work/git/segmentation/networks/mrtf2/resize_image_"+idx+".dat")
...
def load_mrcnn_weights(model, weights_path, verbose=True):
    ...
    if config['training']:
        ...
        # Load training model checkpoint
        try:
            print("XXXXXXXXX Loading model weights XXXXXXXXXXXX")
            training_model.load_weights(weights_path) #.expect_partial()
