...
class MaskRCNN(tf.keras.Model):
    ...
    def update_loss_tracker(self, rpn_class_loss_val, rpn_bbox_loss_val, mrcnn_class_loss_val,
                            mrcnn_bbox_loss_val, mrcnn_mask_loss_val, l2_reg_loss_val, loss, rpn_fg_loss_val, rpn_bg_loss_val, rpn_fg_num, rpn_bg_num, per_class_loss_dict) -> None:
        for loss_name, loss_value in zip(('rpn_class_loss', 'rpn_bbox_loss', 'mrcnn_class_loss', 'mrcnn_bbox_loss',
                                          'mrcnn_mask_loss', 'l2_regularizer', 'loss_sum', 'rpn_fg_class_loss', 'rpn_bg_class_loss', 'rpn_fg_num_instances', 'rpn_bg_num_instances'
                                          ),
                                         (rpn_class_loss_val, rpn_bbox_loss_val, mrcnn_class_loss_val,
                                          mrcnn_bbox_loss_val, mrcnn_mask_loss_val, l2_reg_loss_val, loss, rpn_fg_loss_val, rpn_bg_loss_val, rpn_fg_num, rpn_bg_num)
                                         ):
            if isinstance(loss_value, float) or (loss_value):
                if loss_name not in self.losses_tracker_dict.keys():
                     self.losses_tracker_dict.update({loss_name: tf.keras.metrics.Mean(name=loss_name)})
                self.losses_tracker_dict[loss_name].update_state(loss_value)

        for cls_name, loss_value in per_class_loss_dict.items():
            # import pdb
            # pdb.set_trace()
            loss_name = 'mrcnn_cls_'+cls_name
            if loss_name not in self.losses_tracker_dict.keys():
                self.losses_tracker_dict.update({loss_name: tf.keras.metrics.Mean(name=loss_name)})
            self.losses_tracker_dict[loss_name].update_state(loss_value)
    ...
    def train_step(self, inputs: list) -> dict:
        ...
        self.update_loss_tracker(rpn_class_loss_val, rpn_bbox_loss_val, mrcnn_class_loss_val,
                                 mrcnn_bbox_loss_val, mrcnn_mask_loss_val, l2_reg_loss_val, loss, rpn_fg_loss_val, rpn_bg_loss_val, rpn_fg_num, rpn_bg_num, per_class_loss_dict)
