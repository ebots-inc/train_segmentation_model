...
@gs.Graph.register()
def add_trt_pyramid_roialign(self, inputs_list, attrs, name, clear_outputs=False, verbose=False):
    """
...
def modify_onnx_model(model_path, config, output_names=None, verbose=False):
    ...
    graph.onnx_reshape_layer(input_tensor=graph.tensors()['concat_rpn_class'],
                             reshape=np.array([batch_size, -1, 2, 1]),
                             name='concat_rpn_class_expanded',
                             info_shape=[batch_size, -1, 2, 1]
                             )
    graph.onnx_reshape_layer(input_tensor=graph.tensors()['concat_rpn_bbox'],
                             reshape=np.array([batch_size, -1, 4, 1]),
                             name='concat_rpn_bbox_expanded',
                             info_shape=[batch_size, -1, 4, 1]
                             )
    # pdb.set_trace()
    config['image_shape'] = [512,512,3]
    print(graph.tensors()['concat_rpn_class_expanded'].shape, config['rpn_nms_threshold'], (config['image_shape'][2], *config['image_shape'][:2]))
    ...
    graph.add_trt_pyramid_roialign(inputs_list=[graph.tensors()['roi'],
                                   ...
                                   attrs={'legacy': True,'pooled_size': config['pool_size']
                                    },
    ...
    graph.onnx_reshape_layer(input_tensor=graph.tensors()[_input_tensor_fpnclf],
                             reshape=np.array([batch_size, config['post_nms_rois_inference'], 4 * config['num_classes'], 1, 1]),
                             name='mrcnn_bbox_reshaped',
                             info_shape=[batch_size, config['post_nms_rois_inference'], 4 * config['num_classes'], 1, 1]
                             )
    ...
    graph.onnx_reshape_layer(
        ...
        reshape=np.array([batch_size, config['post_nms_rois_inference'], config['num_classes'], 1, 1]),
        name='mrcnn_class_reshaped',
        info_shape=[batch_size, config['post_nms_rois_inference'], config['num_classes'], 1, 1]
    )
    ...
    graph.add_trt_pyramid_roialign(inputs_list=[graph.tensors()['mrcnn_detection_boxes'],
                                    ...
                                    attrs={'legacy': True,'pooled_size': config['mask_pool_size']
                                    },
                                   name='roi_align_mask',
                                   clear_outputs=False,
                                   verbose=verbose
    ...
    if 'mobilenet' in config['backbone']:
        ...
        for node_name in pad_nodes:
            ...
            graph.nodes[node_id].inputs.append(gs.Constant(name='zero_pad_values',
                                                           values=np.array([0, 0, 0, 0, 0, 0, 1, 1])))
