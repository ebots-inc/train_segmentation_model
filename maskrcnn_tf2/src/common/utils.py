...
def generate_anchors_from_xy_scales(x_scales, y_scales, shape, feature_stride, anchor_stride):
    """
    scales: 1D array of anchor sizes in pixels. Example: [32, 64, 128]
    ratios: 1D array of anchor ratios of width/height. Example: [0.5, 1, 2]
    shape: [height, width] spatial shape of the feature map over which
...
def generate_pyramid_anchors(scales, ratios, feature_shapes, feature_strides, anchor_stride, verbose=False):
    ...
    for i in range(len(scales)):
        ...
        anchors.append(a)
        print("scale {}, feature_shapes {}, feature_strides {}, anchor_stride {}".format(scales[i], feature_shapes[i],  feature_strides[i], anchor_stride))
...
def build_rpn_targets(anchors, gt_class_ids, gt_boxes, rpn_train_anchors_per_image, rpn_bbox_std, cable_instances_start_end_indices=None, eps=1e-3):
    ...
    # print("number of anchors for class 1 are selected: ", tf.math.count_nonzero(gt_class_ids==1), tf.math.count_nonzero(gt_class_ids[anchor_iou_argmax[rpn_match==1]]==1))
    # print("number of anchors for class 2 are selected: ", tf.math.count_nonzero(gt_class_ids==2), tf.math.count_nonzero(gt_class_ids[anchor_iou_argmax[rpn_match==1]]==2))
    # print("number of anchors for class 3 are selected: ", tf.math.count_nonzero(gt_class_ids==3), tf.math.count_nonzero(gt_class_ids[anchor_iou_argmax[rpn_match==1]]==3))
    ...
    for (start_idx, end_idx) in cable_instances_start_end_indices:
        ...
        extra = int(0.85*(ids.shape[0]))
        if extra>0:
            # print("Number of actual, extra positive samples: ", end_idx-start_idx, extra)
            extra_ids = np.random.choice(ids, extra, replace=False)
            rpn_match[extra_ids] = 0

        # print("number of chosen anchors for class 1 are selected: ", tf.math.count_nonzero(gt_class_ids==1), tf.math.count_nonzero(gt_class_ids[anchor_iou_argmax[ids]]==1))
        # print("number of chosen anchors for class 2 are selected: ", tf.math.count_nonzero(gt_class_ids==2), tf.math.count_nonzero(gt_class_ids[anchor_iou_argmax[ids]]==2))
        # print("number of chosen anchors for class 3 are selected: ", tf.math.count_nonzero(gt_class_ids==3), tf.math.count_nonzero(gt_class_ids[anchor_iou_argmax[ids]]==3))

    # Subsample to balance positive and negative anchors
    # Don't let positives be more than half the anchors
    ids = np.where(rpn_match == 1)[0]
    extra = len(ids) - (rpn_train_anchors_per_image // 2)
    # print("Number of extra positive samples: ", extra)
    if extra > 0:
    ...
    return rpn_match, rpn_bbox, overlaps


def generate_random_rois(image_shape, count, gt_boxes, gt_class_ids):
    ...
    for i in range(gt_boxes.shape[0]):
        ...
        while True:
            ...
            # else:
            #     total_limit = 12
            #     x_limit =  np.random.randint(0, total_limit)
            #     y_limit =  np.random.randint(0, max(0, total_limit-x_limit))
            #     y1y2 = np.random.randint(0, 5, (rois_per_box * 2, 2))
            #     x1x2 = np.random.randint(0, 5, (rois_per_box * 2, 2))
...
def build_detection_targets(rpn_rois, gt_class_ids, gt_boxes, gt_masks,
    ...
    non_cable_positive_instances = np.logical_and(rpn_roi_iou_max > 0.7, rpn_roi_gt_class_ids!=3)
    # print("NON cable positive instances... ", np.sum(non_cable_positive_instances))
    cable_positive_instances = np.logical_and(rpn_roi_iou_max > 0.8, rpn_roi_gt_class_ids==3)
    # print("cable positive instances... ", np.sum(cable_positive_instances))

    # Positive ROIs are those with >= 0.5 IoU with a GT box.
    fg_ids = np.where(np.logical_or(non_cable_positive_instances, cable_positive_instances))[0]

    # Negative ROIs are those with max IoU 0.1-0.5 (hard example mining)
    # TODO: To hard example mine or not to hard example mine, that's the question
    # bg_ids = np.where((rpn_roi_iou_max >= 0.1) & (rpn_roi_iou_max < 0.5))[0]
    bg_ids = np.where(rpn_roi_iou_max < 0.4)[0]
...
def reformat_detections(detections, mrcnn_mask, original_image_shape, image_shape, window, mask_score_threshold=0.5):
    ...
    for i in range(N):
        ...
        full_mask = unmold_mask(masks[i], boxes[i], original_image_shape, mask_score_threshold)
