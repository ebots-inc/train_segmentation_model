...
class AnchorsLayer(tfl.Layer):
    def __init__(self, config, training, name="anchors", **kwargs):
        ...
        self.batch_anchors = tf.repeat(self.anchors, self.config["batch_size"], 0)
    ...
    def get_anchors(self):
        ...
        if not tuple(self.image_shape) in self._anchor_cache:
            # Generate Anchors
            if 'rpn_anchor_scales' in self.config:
                ...
                self.anchors = utils.generate_pyramid_anchors_from_xy_scales(self.config['rpn_anchor_scales_x'],
                                            self.config['rpn_anchor_scales_y'], backbone_shapes,
                                            self.config['backbone_strides'],
                                            self.config['rpn_anchor_stride'])
        ...
        # np.broadcast_to(anchors, (self.config['batch_size'],) + anchors.shape)
        return np.broadcast_to(anchors, (1,) + anchors.shape)
...
def detection_targets_graph(proposals, gt_class_ids, gt_boxes, gt_masks, config):
    ...
    # if tf.size(positive_roi_bool)>0:
    #     print("positive roi bool: ", tf.math.count_nonzero(positive_roi_bool), tf.size(positive_indices))
    #     print("Class 1 count: ", tf.math.count_nonzero(roi_gt_class_ids==1))
    #     print("Class 2 count: ", tf.math.count_nonzero(roi_gt_class_ids==2))
    #     print("Class 3 count: ", tf.math.count_nonzero(roi_gt_class_ids==3))

    # else:
    #     print("Zero positive rois are seen")
...
class MaskRCNNBackbone:
    def __init__(self, config):
        ...
        self._backbone_list = ['resnet18', 'resnet34', 'resnet50', 'resnet101', 'resnet152', 'mobilenet', 'mobilenetv2',
                               ...
                               'efficientnetb5', 'efficientnetb6', 'efficientnetb7', 'efficientnetv2S', 'efficientnetv2M', 'efficientnetv2L', 'seresnet18', 'seresnet34',
        ...
        self.backbone_outputs.update({
            f'efficientnetb{i}': ['block2a_expand_activation', 'block3a_expand_activation', 'block4a_expand_activation',
                                  'block6a_expand_activation', 'block7b_expand_activation'] for i in range(8)})

        self.backbone_outputs.update({
            f'efficientnetv2{i}': ['block2a_expand_activation', 'block2e_expand_activation', 'block4a_expand_activation',
                                  'block6a_expand_activation', 'block7e_expand_activation'] for i in ['S', 'M', 'L']})
    ...
    def _get_notop_model(self):
        ...
        _effnet_mapping = {'efficientnetb0': efficientnet.EfficientNetB0, 'efficientnetb1': efficientnet.EfficientNetB1,
                    'efficientnetb2': efficientnet.EfficientNetB2, 'efficientnetb3': efficientnet.EfficientNetB3,
                    'efficientnetb4': efficientnet.EfficientNetB4, 'efficientnetb5': efficientnet.EfficientNetB5,
                    'efficientnetb6': efficientnet.EfficientNetB6, 'efficientnetb7': efficientnet.EfficientNetB7, }

        _effnetv2_mapping = {'efficientnetv2S': efficientnet_v2.EfficientNetV2S, 'efficientnetv2M': efficientnet_v2.EfficientNetV2M, 'efficientnetv2L': efficientnet_v2.EfficientNetV2L}
        ...
        if self._match_by_pattern(patterns=[r'efficientnetb\d+']):
            ...
            model = _effnet_mapping[self.backbone_name](input_shape=self.input_shape, weights=self.weights,
                                                        include_top=False)
        elif self._match_by_pattern(patterns=[r'efficientnetv2+']):
            # EfficientNets
            model = _effnetv2_mapping[self.backbone_name](input_shape=self.input_shape, weights=self.weights,
                                                        include_top=False)
    ...
    def _choose_backbone(self):
        ...
        if not self.config['train_bn_backbone']:
            ...
            for layer_name in bn_layers:
                print("Batch norm layers: ",layer_name)
